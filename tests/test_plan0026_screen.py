...
from scripts import plan0026_screen as screen
...
class ScreenTests(unittest.TestCase):
    ...
    def test_schedule_is_exactly_prospective_and_saved_before_first_call(self):
        ...
        self.assertEqual(
            ...
            (
                "docs/plans/active/0026-two-forward-orthogonal-hunters-screen.md",
                (
                    "experiments/proposals/"
                    "three-runners-two-forward-orthogonal-hunters-v0.json"
                ),
                "plan0026-two-forward-orthogonal-hunters-v1",
                "aed723e270cacd8bdfe3c87db22299d8ccf1045e63c0cebbd638d900d0491b94",
                (
                    "PLAN0025_OUTCOME_AND_TRACE_GUIDED_DEVELOPMENT_AND_"
                    "PREREGISTRATION_BOUNDED_AND_OR_EXPOSED_NOT_CONFIRMATION"
                ),
        ...
        self.assertEqual(
            ...
            list(range(700, 716)) * 2
            + list(range(800, 816))
            + list(range(800, 808)) * 4,
        )
    ...
    def test_exact_short_and_base_cascade_gates(self):
        ...
        for mode, disposition, expected_games in cases:
            ...
            def play(raw, policy_a, policy_b, seed, cap):
                result = game(raw, policy_a, policy_b, seed, cap)
                if mode == "short_loss" and policy_a == policy_b == "T4" and seed == 700:
                    result["decisions"][0]["last_action_values"] = [
                        {"value": -1}, {"value": -1}
                    ]
                elif mode == "short_win" and policy_a == policy_b == "T4" and seed == 700:
                    result["decisions"][0]["last_action_values"] = [
                        {"value": 0}, {"value": 1}
                    ]
                elif mode == "base_t4_low" and policy_a == policy_b == "T4":
                    result["winner"] = "A" if seed < 704 else "B"
                elif mode == "base_t4_high" and policy_a == policy_b == "T4":
                    result["winner"] = "A" if seed < 712 else "B"
                elif mode == "base_t3_low" and policy_a == policy_b == "T3":
                    result["winner"] = "A" if seed < 703 else "B"
                elif mode == "base_t3_high" and policy_a == policy_b == "T3":
                    result["winner"] = "A" if seed < 713 else "B"
                elif mode == "base_censor" and policy_a == policy_b == "T4" and seed == 700:
    ...
    def test_opened_final_block_runs_all_games_even_if_censored(self):
        def play(raw, policy_a, policy_b, seed, cap):
            if (policy_a, policy_b, seed) == ("R", "T4", 800):
